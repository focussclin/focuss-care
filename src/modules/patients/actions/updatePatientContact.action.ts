...
import {
  ...
  type UpdatePatientContactInput,
...
const runUpdatePatientContact = createAction<
  UpdatePatientContactInput,
  PatientContactDto,
  PatientContactField
>({
  name: 'patient.contact.update',
  schema: updatePatientContactSchema,
  ...
  handler: async (input, context) => {
    ...
    try {
      ...
      const contact = await contacts.update(context.clinicId, patient.id, input.contactId, {
        name: input.name,
  ...
  audit: (output) => ({
    action: 'patient.contact.updated',
...
export async function updatePatientContactAction(
  ...
  return runUpdatePatientContact(rawInput)
}
